...
#######################
'''
*Volumes has been changed to array key  but as far as I can tell is the same thing for now

*add_volume_request has been changed to simply add but as far as I can tell does the same thing

*Tensorflow implementation does not use solver_parameters the same way as caffe so still need to sort out how to specify all those
parameters in the most neat way
 
*net.prototxt is the caffee version of the metagraph tensor flow needs, need to convert using 
"https://github.com/ethereon/caffe-tensorflow" I will try to verify the conversion using the tensorboard visualizer.

*Need to look into Docker
...
def train():
    ...
    snapshot_request.add(ArrayKeys.LOSS_GRADIENT, (56, 56, 56))
    ...
    batch_provider_tree = (
        ...
        #Check train line when debugging first if there are issues !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
